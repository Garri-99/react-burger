...
interface IOrderPageProp {
  isModal?: boolean;
  isUser?: boolean;
}

const OrderPage: FC<IOrderPageProp> = ({ isModal, isUser }) => {
  ...
  }, [dispatch, wsInit, isUser]);
  const { ingredients } = useSelector((store) => store.ingredients);
  const order = useSelector(
    (store) => store.socket[isUser ? "myOrders" : "orders"]
  ).find((i) => i._id === id);
  ...
  return order ? (
    <div className={style.wrapper}>
      ...
      <p className="text text_type_main-default mb-15" style={{ color: color }}>
        {status}
      </p>
